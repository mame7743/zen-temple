...
@click.option(
    '--no-examples',
    is_flag=True,
    help='Skip creating example components'
)
@click.option(
    '--with-server',
    is_flag=True,
    help='Include a basic Flask development server'
)
def new(project_name: str, path: str, no_examples: bool, with_server: bool) -> None:
    ...
    try:
        ...
        if with_server:
            ...
            click.echo(f"  python app/main.py")
        else:
            click.echo(f"  # Edit templates in templates/ directory")
            click.echo(f"  # View zen-temple.yaml for configuration")
...
@click.option(
    '--output',
    type=click.Path(),
    help='Output directory (default: templates/components)'
)
